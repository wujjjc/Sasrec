...
def read_data():
    ...
    :return:
        user_info: 用户信息字典，结构为：
            {
                user_id(str): (male(int), age(int), occupation(int)),  # 性别（男1女0）、年龄、职业编码
            }
        ...
        max_age: int, 用户年龄的最大值
        max_occupation: int, 用户职业编码的最大值
    """
    ...
    with open('./ml-1m/users.dat', 'r', encoding='latin-1') as f:
        for line in f:
            ...
            user_info[int(user_id)] = (int(gender == 'M'), int(age), int(occupation))
            max_age = max(max_age, int(age))
            max_occupation = max(max_occupation, int(occupation))
    ...
    print("读取完成")
    return user_info, movie_info, rating_info, max_age, max_occupation
...
def gen_input(user_item_dict, movie_info, max_len, neg_sample_num=3, user_info=None):
    """
    ...
    :param user_info: 用户信息字典，格式为 {user_id: (gender: int, age: int, occupation: int), ...}
    :return:
        ...
        trn_click: list[dict]                                     #验证集,
            ...
            'pos_item': int           # 答案
            'neg_items': np.ndarray (neg_sample_num,) # 负样本物品ID（编码后）
        all_item: np.ndarray (item_num,)              # 所有物品的编码ID列表（不含padding索引0）
    """
    print("开始处理数据")
    ...
    all_item = np.arange(1, item_num, dtype=np.int64)  # 编码后ID，跳过padding的0
    ...
    for user_id, seq in tqdm(processed_seq_dict.items()):
        ...
        gender, age, occupation = user_info[user_id] #获取用户信息，默认值为0
        for i in range(len(seq)):
            ...
            sample.append({'item_seq': item_seq, 'seq_len': len_seq, 'pos_item': pos_item, 'neg_items': neg_items,
                           'gender': gender, 'age': age, 'occupation': occupation})
    print("输入处理完成")
    return sample, item_num, item_index_2_rawid, trn_click, all_item
...
def recommend(item_embbeding, output, user_id, train_item_seq, item_index_2_rawid, neg_items, target_id, topk=10):
    ...
    # candidate_ids 是全量编码后ID，target_id 是原始ID，统一转为原始ID做比较
    test_rawitems = {item_index_2_rawid.get(int(cid)) for cid in neg_items}
