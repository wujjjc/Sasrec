...
class MLP(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, dropout):
        ...
        dense = []
        for hide in hidden_size:
            dense.append(nn.Linear(input_size, hide))
...
class Sasrec_Net(nn.Module):
    def __init__(self, embedding_size, embedding_num, max_len, mode=None, dropout=0.2, max_age=100, max_occupation=50):
        super(Sasrec_Net, self).__init__()
        self.embedding_size = embedding_size
        self.max_len = max_len
        self.embedding = Embedding_layer(embedding_num, embedding_size)
        self.gender_embedding = nn.Embedding(2, embedding_size) #0-1编码
        self.age_embedding = nn.Embedding(max_age + 1, embedding_size) #年龄编码
        self.occupation_embedding = nn.Embedding(max_occupation + 1, embedding_size) #职业编码
        ...
        self.mode = mode
        self.mlp = MLP(embedding_size * 3, [embedding_size * 2], embedding_size, dropout)
    ...
    def forward(self, item_seq, mask, gender, age, occupation, neg_item=None, pos_item=None): #item_train表示正负样本的连接
        ...
        user_emb = self.mlp(torch.cat([gender_emb, age_emb, occupation_emb], dim=-1)).unsqueeze(1) #batch, 1, embedding_size
        seq_len = item_seq.shape[1]
        positions = torch.arange(seq_len, device=item_seq.device).unsqueeze(0)  # (1, seq_len), 位置编码
        embedding = self.dropout(torch.cat([user_emb, self.embedding(item_seq) + self.pos_embedding(positions)], dim=1)) #batch, n+1, embedding_size
